    ...
    try {
      ...
      if (data && data.status === 'failed') {
        ...
        if (data.fcmNote === 'fcm-unconfigured') {
          errorMsg = 'Push is disabled on the server. Configure FIREBASE_SERVICE_ACCOUNT_B64 in the Render backend environment to enable FCM.';
        } else if (data.fcmNote === 'fcm-rejected') {
          errorMsg = 'FCM rejected the token(s). The receiver may need to reinstall/clear data for a fresh push token.';
